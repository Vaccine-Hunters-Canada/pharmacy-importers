...
HEADERS = {
    ...
    "origin": "https://metropharmacy.medmeapp.com",
    "referer": "https://metropharmacy.medmeapp.com/",
...
class METROPharmacy:
    """
    Represents a single instance of a Metro pharmacy
    ...
    def get_external_key(pharmacy):
        ...
        return f"metro-{pharmacy['storeNo']}"
    ...
    def external_key(self):
        return METROPharmacy.get_external_key(self.pharmacy)
    ...
    @property
    def website(self):
        return f"https://metropharmacy.medmeapp.com/{self.pharmacy['storeNo']}/schedule/"
...
async def main():
    async with aiohttp.ClientSession(headers=HEADERS) as session:
        ...
        vhc = VHC(
            ...
            org_id=os.environ.get("VHC_ORG_METRO"),
            session=session,
        )

        # Generate a lookup of external ID to Metro pharmacy
        # As we see each pharmacy, add it to the lookup if it isn't yet there
        # Then, update its tags and availability
        pharmacies: dict[str, METROPharmacy] = {}
        ...
        for vaccine_data in VACCINES.values():
            for pharmacy_data in await get_available_pharmacies(
                ...
                external_key = METROPharmacy.get_external_key(pharmacy_data)

                # Get or create the METROPharmacy instance
                if external_key in pharmacies:
                    pharmacy = pharmacies[external_key]
                else:
                    pharmacy = METROPharmacy(pharmacy_data)
                    pharmacies[external_key] = pharmacy

                # Update it with values from this data
                pharmacy.available |= METROPharmacy.is_available(pharmacy_data)
